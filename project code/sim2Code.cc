...
#include "ns3/system-path.h"      // <-- for SystemPath::MakeDirectories
...
uint64_t prevTxBytes1 = 0;
uint64_t prevTxBytes2 = 0;

Ptr<Ipv4FlowClassifier> classifier;

// MSS for cwnd printing in MSS units
...
{
    // ----- Timestamp text (unused in path but handy) -----
    char buffer[80];
    ...
    // ===== CLI params =====
    double rttMs = 0.0;                 // legacy single-RTT mode
    double edgeDelayMs = 5.0;           // default edges when not using RTT controls
    double bottleneckDelayMs = 10.0;    // default bottleneck one-way delay
    std::string dataRateEdge = "1000Mbps";
    std::string dataRateBottleneck = "10Mbps";

    uint32_t mssBytes = 1448;
    double start1Sec = 0;
    double start2Sec = 0;

    // TCP defaults
    std::string tcp1 = "ns3::TcpNewReno";
    std::string tcp2 = "ns3::TcpNewReno";
    uint32_t delAckCount = 2;
    Time stopTime = Seconds(120);

    // NEW: per-flow RTT controls
    double rtt1Ms = 0.0;       // target RTT (ms) for Flow 1
    double rtt2Ms = 0.0;       // target RTT (ms) for Flow 2 (overrides rtt2Factor if >0)
    double rtt2Factor = 1.0;   // if rtt2Ms==0, RTT2 = rtt2Factor * rtt1Ms

    CommandLine cmd(__FILE__);
    cmd.AddValue("rttMs", "Total desired RTT in ms (legacy; common).", rttMs);
    cmd.AddValue("edgeDelayMs", "Edge one-way delay in ms (used if rttMs==0 and per-flow RTTs disabled).", edgeDelayMs);
    cmd.AddValue("bottleneckDelayMs", "Bottleneck one-way delay in ms.", bottleneckDelayMs);
    ...
    // NEW CLI bindings
    cmd.AddValue("rtt1Ms", "Target RTT (ms) for Flow 1.", rtt1Ms);
    cmd.AddValue("rtt2Ms", "Target RTT (ms) for Flow 2 (overrides rtt2Factor).", rtt2Ms);
    cmd.AddValue("rtt2Factor", "RTT2 = rtt2Factor * rtt1Ms if rtt2Ms==0.", rtt2Factor);
    ...
    // ---------- Delay computation ----------
    // RTT_i ≈ 4*E_i + 2*B
    double E1 = edgeDelayMs;
    double E2 = edgeDelayMs;
    double B  = bottleneckDelayMs;

    if (rtt1Ms > 0.0)
    {
        if (rtt2Ms <= 0.0) {
            rtt2Ms = (rtt2Factor > 0.0 ? rtt2Factor * rtt1Ms : rtt1Ms);
        }

        auto clampRtt = [B](double rtt) { return std::max(rtt, 2.0 * B); };
        rtt1Ms = clampRtt(rtt1Ms);
        rtt2Ms = clampRtt(rtt2Ms);

        E1 = std::max(0.0, (rtt1Ms - 2.0 * B) / 4.0);
        E2 = std::max(0.0, (rtt2Ms - 2.0 * B) / 4.0);

        double rtt1_est = 4.0 * E1 + 2.0 * B;
        double rtt2_est = 4.0 * E2 + 2.0 * B;
        std::cout << "[RTT] Using per-flow RTTs: B(one-way)=" << B
                  << " ms, E1=" << E1 << " ms, E2=" << E2
                  << " ms  => RTT1≈" << rtt1_est << " ms, RTT2≈" << rtt2_est << " ms\n";
    }
    else if (rttMs > 0.0)
    {
        // legacy: edge:bneck = 1:2 ⇒ RTT≈8*edge
        double edge = rttMs / 8.0;
        E1 = E2 = edge;
        B  = 2.0 * edge;
    }

    // ---------- Helpers configured with the computed delays ----------
    ...
    bottleneckLink.SetChannelAttribute("Delay",  StringValue(std::to_string(B)   + "ms"));

    edgeLinkSender.SetDeviceAttribute("DataRate", StringValue(dataRateEdge));
    edgeLinkSender.SetChannelAttribute("Delay",  StringValue(std::to_string(E1)  + "ms"));
    edgeLinkSender2.SetDeviceAttribute("DataRate", StringValue(dataRateEdge));
    edgeLinkSender2.SetChannelAttribute("Delay", StringValue(std::to_string(E2) + "ms"));

    edgeLinkReceiver.SetDeviceAttribute("DataRate", StringValue(dataRateEdge));
    edgeLinkReceiver.SetChannelAttribute("Delay",  StringValue(std::to_string(E1) + "ms"));
    edgeLinkReceiver2.SetDeviceAttribute("DataRate", StringValue(dataRateEdge));
    edgeLinkReceiver2.SetChannelAttribute("Delay", StringValue(std::to_string(E2) + "ms"));
    ...
    // ----- QueueDisc at bottleneck (R1 side, device index 2) -----
    TrafficControlHelper tch;

    // Simple FIFO (no ECN/RED)
    tch.SetRootQueueDisc("ns3::FifoQueueDisc");

    // OPTIONAL: reproducible queue size
    // Config::SetDefault("ns3::FifoQueueDisc::MaxSize", StringValue("100p"));
    ...
    if (tcR1) { qdiscR1 = tcR1->GetRootQueueDiscOnDevice(devR1); }
    if (!qdiscR1)
    {
    ...
    // Optional: save flowmon XML
    // monitor->SerializeToXmlFile(dir + "/flowmon.xml", true, true);
